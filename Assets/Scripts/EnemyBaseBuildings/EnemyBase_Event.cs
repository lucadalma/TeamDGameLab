...
public class EnemyBase_Event : MonoBehaviour
{
    EnemyBase_EventManager EM;
    public BuildingButton.forUnit unitToUpgrade;

    BuildingCategorization BC;

    EnemyBase_StackEvent SE;
    ...
    {
        if (EM == null)
            EM = FindObjectOfType<EnemyBase_EventManager>();
        if (SE == null)
            SE = FindObjectOfType<EnemyBase_StackEvent>();
        if (BC == null)
